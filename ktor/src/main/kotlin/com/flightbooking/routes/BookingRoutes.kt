...
fun Route.bookingRoutes() {
    ...
    route("/booking") {
        post("/create_booking") {
            ...
            val flightCode =
                params["flight_code"]
                    ?: return@post call.respond(HttpStatusCode.BadRequest, "Missing flight_code")
            val returnFlightCode = params["return_flight_code"]

            val flight =
                flightRepository.getFlightByFlightCode(flightCode)
                    ?: return@post call.respond(HttpStatusCode.NotFound, "Flight not found")
            val flightId = flight.id
            val returnFlightId =
                if (returnFlightCode != null) {
                    val returnFlight =
                        flightRepository.getFlightByFlightCode(returnFlightCode)
                            ?: return@post call.respond(HttpStatusCode.NotFound, "Return Flight not found")
                    returnFlight.id
                } else {
                    null
                }
        ...
        get("/{reference}/passengers") {
            ...
            val booking =
                bookingRepository.getBookingByReference(reference)
        ...
        post("/{reference}/passengers") {
            ...
            val reference =
                call.parameters["reference"]
                    ?: return@post call.respond(HttpStatusCode.BadRequest, "Missing booking reference")
            val params = call.receiveParameters()

            val booking =
                bookingRepository.getBookingByReference(reference)
                    ?: return@post call.respond(HttpStatusCode.NotFound, "Booking not found")
            if (booking.userId != session.userId) {
        ...
        post("/{reference}/ticket_assignment") {
            ...
            val flight =
                flightRepository.getFlightByFlightId(booking.flightId)
                    ?: return@post call.respond(HttpStatusCode.NotFound, "Flight not found")

            val returnFlight =
                if (booking.returnFlightId != null) {
                    flightRepository.getFlightByFlightId(booking.returnFlightId)
                        ?: return@post call.respond(HttpStatusCode.NotFound, "Return Flight not found")
                } else {
                    null
                }
            ...
            try {
                transaction {
                    ...
                    for (passenger in passengers) {
                        val flightSeatId =
                            params["passenger_${passenger.id}_seat_id"]?.toIntOrNull()
                                ?: throw Exception("Missing flight seat id for passenger ${passenger.id}")
                        val seatClass =
                            bookingRepository.getSeatClassByFlightSeatId(flightSeatId)
                                ?: throw Exception("Outbound Seat Class not found")
                        val seatNumber =
                            bookingRepository.getSeatNumberByFlightSeatId(flightSeatId)
                                ?: throw Exception("Outbound Seat Number not found")
                        ...
                        if (returnFlight != null) {
                            val returnFlightSeatId =
                                params["passenger_${passenger.id}_return_seat_id"]?.toIntOrNull()
                                    ?: throw Exception("Missing return flight seat id for passenger ${passenger.id}")
                            val returnSeatClass =
                                bookingRepository.getSeatClassByFlightSeatId(returnFlightSeatId)
                                    ?: throw Exception("Return Seat Class not found")
                            val returnSeatNumber =
                                bookingRepository.getSeatNumberByFlightSeatId(returnFlightSeatId)
                                    ?: throw Exception("Return Seat Number not found")
        ...
        get("/{reference}/payment") {
            ...
            if (price == BigDecimal.ZERO) {
                return@get call.respond(HttpStatusCode.InternalServerError, "Booking price zero, mistake made")
            }
            // call.respond(price)
        ...
        post("/{reference}/payment") {
            ...
            if (booking.createdAt.plusMinutes(30) < LocalDateTime.now()) {
                ...
                return@post call.respondPebble(
                    "index.peb",
                    mapOf("error" to "Your booking session expired. Please search again."),
                )
            }
            ...
            if (amount == BigDecimal.ZERO) {
                return@post call.respond(
                    ...
                    "Booking amount zero, mistake made",
                )
            }
            val paymentMethodParam =
                params["payment_method"]
                    ?: return@post call.respond(HttpStatusCode.BadRequest, "Missing payment method")
            ...
            if (confirmed != 1) {
                return@post call.respond(HttpStatusCode.InternalServerError, "Couldn't confirm booking")
            }

            // call.respond(payment)
            call.respondPebble(
                "paymentConfirmation.peb",
                mapOf("payment" to payment, "addedPoints" to addedPoints, "reference" to reference),
            )
        }
        ...
        post("/{reference}/send_tickets") { // button on paymentConfirmation.peb
            ...
            val user =
                userRepository.getUserById(session.userId)
                    ?: return@post call.respond(HttpStatusCode.NotFound, "Logged in user not found")
        ...
        post("/{reference}/cancel") {
            ...
            val cancelledBooking =
                bookingRepository.cancelBooking(reference)
                    ?: return@post call.respond(HttpStatusCode.InternalServerError, "Could not cancel booking")
