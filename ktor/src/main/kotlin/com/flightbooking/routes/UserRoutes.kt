...
fun Route.userRoutes() {
    ...
    route("/register") {
        ...
        post {
            ...
            if (atIndex <= 0 || email.indexOf('.', atIndex) <= atIndex + 1 || email.last() == '.') {
                return@post call.respondPebble(
                    "getemail.peb",
                    mapOf("error" to "Email isn't in valid format"),
                ) // shouldnt happen unless frontend is bypassed
            }
        ...
        get("/verify") {
            val vfySession =
                call.sessions.get<VerificationSession>()
                    ?: return@get call.respondRedirect("/register")
        ...
        post("/verify") {
            ...
            val otpParam =
                params["otp_param"]?.trim()
                    ?: return@post call.respondPebble("verify.peb", mapOf("error" to "No code entered"))
        ...
        get("/details") {
            val vfySession =
                call.sessions.get<VerificationSession>()
                    ?: return@get call.respondRedirect("/register")
            if (vfySession.verified != true) {
                return@get call.respondRedirect("/register/verify")
            }
        ...
        post("/details") {
            val vfySession =
                call.sessions.get<VerificationSession>()
                    ?: return@post call.respondPebble("register.peb", mapOf("error" to "No verification session"))
            val params = call.receiveParameters()
    ...
    post("/login") {
        ...
        if (email == null || password == null) {
            ...
            return@post call.respondPebble(
                "login.peb",
                mapOf("error" to "Missing email or password"),
            ) // shouldnt happen unless frontend is bypassed
        }
